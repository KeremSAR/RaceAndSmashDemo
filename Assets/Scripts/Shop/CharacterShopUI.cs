...
public class CharacterShopUI : MonoBehaviour
{
	...
	[Header("UI elements")]
	[SerializeField]
	RawImage selectedCharacterIcon;
	[SerializeField]
	Transform ShopMenu;
	[SerializeField]
	Transform ShopItemsContainer;
	[SerializeField]
	GameObject itemPrefab;
	[Space(20)]
	[SerializeField]
	CharacterShopDatabase characterDB;
	...
	[Space (20)]
	[Header ("Main Menu")]
	[SerializeField] RawImage mainMenuCharacterImage;
	...
	void GenerateShopItemsUI()
	{
		...
		for (int i = 0; i < characterDB.CharactersCount; i++)
		{
			...
			uiItem.SetCharacterImage(character.texture);
			//uiItem.SetCharacterSpeed(character.speed);
			uiItem.SetCharacterPower(character.health);
	...
	void ChangePlayerSkin ()
	{
		...
		if (character.texture != null) {
			...
			mainMenuCharacterImage.texture = character.texture;
			mainMenuCharacterName.text = character.name;

			//Set selected Character Image at the top of shop menu
			selectedCharacterIcon.texture = GameDataManager.GetSelectedCharacter ().texture;
		}
